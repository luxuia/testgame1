...
public class ModsManager
{
    ...
    private void LoadPrototypes()
    {
        ...
        if (!prototypeDir.Exists)
        {
            UnityDebugger.Debugger.LogError("ModsManager", "Prototypes directory not found: " + prototypesDirectoryPath);
            return;
        }
        ...
        {
            string modPrototypesPath = Path.Combine(mod.FullName, "Prototypes");
            if (!Directory.Exists(modPrototypesPath))
            {
                continue;
            }
    ...
    private void LoadPrototypeFileLua(FileInfo file, Dictionary<string, JToken> tagNameToProperty)
    {
        ...
        try
        {
            ...
            if (ret != null && !ret.IsNil() && ret.Type == DataType.Table)
            {
                JToken protoToken = LuaPrototypeConverter.DynValueToJToken(ret);
                if (protoToken != null && protoToken is JObject jobject && jobject.First != null)
                {
                    string tagName = ((JProperty)jobject.First).Name;
                    tagNameToProperty[tagName] = protoToken;
                }
            }
            else
            {
                UnityDebugger.Debugger.LogWarning("ModsManager", "Lua prototype " + file.Name + " did not return a table");
            }
        }
        catch (Exception ex)
        {
            UnityDebugger.Debugger.LogError("ModsManager", "Failed to load Lua prototype " + file.FullName + ": " + ex.Message);
        }
    }
